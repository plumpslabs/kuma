...
export function generateInitMdContent(): string {
  return [
    ...
    "  7. kuma_memory    → arch_flow           — After tracing COMPLETE flow (max 5 core files)",
    "  8. kuma_memory    → feature             — ⭐ When discovering a module/feature boundary (Authentication, Billing)",
    "  9. kuma_memory    → decision            — When choosing (preserves rationale)",
    " 10. kuma_safety    → verify              — Run tests / confirm nothing broken",
    " 11. kuma_context   → changes             — Review session activity",
    "",
    "🔥 `arch_flow` + `gotcha` are EXPONENTIAL — each record saves 5-10 files next session.",
    "⭐ `feature` captures module ownership — use when you identify a cohesive feature boundary.",
    "🟢 SKIP recording function/class/component nodes — grep/glob is faster.",
...
function getCombinedAgentsMd(selectedTypes: Set<ConfigType>): string {
  ...
  "  7. `[kuma_]memory({ action: \"arch_flow\" })`                 — After tracing COMPLETE flow (max 5 core files)",
