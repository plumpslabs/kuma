...
export function generateInitMdContent(): string {
  return [
    ...
    "```",
    "domain: <DomainName> | hops: <file1.tsx> → <file2.js> → <file3.ts> | gotchas: <optional> | decisions: <optional>",
    "```",
    "- `domain:` — short domain name (e.g. `InviteUserFlow`, `LeadManagementFlow`)",
    "- `hops:` — file names separated by ` → ` (NOT newlines, NOT commas)",
    ...
    "| `federated` | Resolve kuma:// URIs | Cross-project references |",
    ...
    "| `clear` | Wipe ALL graph nodes, edges, gotchas, trajectories | Emergency reset — irreversible |",
